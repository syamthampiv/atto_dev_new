...
SELECT  a.requester_id as source_id,
	...
	'REQUEST_ACCREDITATION' as activity_type
        from accreditation a inner join users b on  (a.requester_id = b.id)
                              inner join company c on (b.company_id = c.id)
                              inner join user_role d on (b.user_role = d.id)
		                      left join assets f ON (c.assets -> 'PROFILE_IMAGE'::text) = f.id::character varying::text
		                      inner join users g on  (a.approver_id = g.id)
                              inner join company h on (g.company_id = h.id)
                              inner join user_role i on (g.user_role = i.id)
